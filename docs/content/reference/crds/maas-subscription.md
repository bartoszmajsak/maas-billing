...
!!! note "Subscriptions stored before these limits"
    ...
    - For a reference with no token budget, API servers before Kubernetes 1.33 also reject the controller's status update (a `StatusUpdateRejected` warning event), so its phase may still read `Active`. Inference on the affected model is denied regardless.
